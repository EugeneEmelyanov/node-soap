...
function splitNSName(nsName) {
	var i = (nsName != null && typeof nsName.indexOf !== 'undefined') ? nsName.indexOf(':') : -1;
...
OperationElement.prototype.description = function (definitions) {
	var inputDesc = this.input.description(definitions),
		outputDesc = this.output ? this.output.description(definitions) : "";
	return {
		...
		self.processIncludes(function (err) {
			...
			for (var bindingName in bindings) {
				...
				var methods = binding.methods,
					topEls = binding.topElements = {},
					inputName, outputName;
				for (var methodName in methods) {
					inputName = methods[methodName].input.$name;
					outputName = methods[methodName].output ? methods[methodName].output.$name : '';
					topEls[inputName] = {"methodName": methods[methodName].input.$name, "outputName": outputName};
				}
			}
